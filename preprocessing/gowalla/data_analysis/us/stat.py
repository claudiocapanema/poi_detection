...
def users_events(users):

    median = users.groupby("userid").apply(lambda e: pd.DataFrame({'median': [len(e)]}))
...
if __name__ == "__main__":
    ...
    events_stats = df.groupby("state_name").apply(lambda e: users_events(e)).sort_values('median', ascending=False).reset_index()
    ...
    \nMediana de eventos por usuário por estado: {}""".format(n, n_users, categories, n_countries, top_countries.head(10).reset_index(drop=True), top_countries_users.head(10).reset_index(drop=True), top_countries_users_duration.head(10).reset_index(drop=True), events_stats.head(10).reset_index(drop=True))
