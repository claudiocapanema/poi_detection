...
from configuration import BASE_DIR, CHECKINS, SPOTS_1, CATEGORY_STRUCTURE, TIMEZONES, BRASIL_STATES, COUNTRIES, CHECKINS_LOCAL_DATETIME, CHECKINS_5_CATEGORIES_OSM
...
if __name__ == "__main__":
    ...
    super_categories_dict = {'Other - Travel & Lodging': 'Travel', 'Urban Outfitters': 'Shopping', 'Starbucks': 'Food', 'Walmart': 'Shopping', 'Nike': 'Shopping',
                             ...
                             'Walt Disney World Resort': "Travel", "Dunkin' Donuts": "Food",  'Disneyland Resort': "Travel", 'Coffee Bean and Tea Leaf': "Food",
                             }
    ...
    print("categorias", super_categories_dict)

    df_category_subcategory = pd.DataFrame({'category': list(super_categories_dict.values()), 'subcategory': list(super_categories_dict.keys())}).sort_values('category')

    data = ""
    df_category_subcategory = pd.read_csv("gowalla_category_subcategory_osm_category.csv")
    subcategories = df_category_subcategory['subcategory'].tolist()
    categories = df_category_subcategory['osm_category'].tolist()
    super_categories_dict = {subcategories[i]: categories[i] for i in range(len(subcategories))}
    print("leu", super_categories_dict)
    ...
    print("fez o join: ", checkins)

    #checkins['subcategory'] = get_subcategory(checkins['category'].tolist())
    checkins['category'] = to_super_category(checkins['category'].tolist(), super_categories_dict)

    print(checkins)

    print("categorias únicas: ")
    checkins = checkins.dropna(subset=['category'])
    checkins = checkins.query("category != 'nan'")
    print(checkins['category'].unique().tolist())



    checkins.to_csv(CHECKINS_5_CATEGORIES_OSM, index_label=False, index=False)
