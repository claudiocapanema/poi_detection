...
class Capanema(Model):
    ...
    def gnn_weight(self, gnn, categories):
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        components = inputs
        score_gnn = kwargs['score']
        score_rnn = 1 - score_gnn
        ...
        total_entropy = tf.reduce_sum(entropies)
        ...
        out_entropy = tf.reduce_mean(tfp.distributions.Categorical(probs=out_sum).entropy())
        self.entropy_list.append(out_entropy)

        size = 0
        total = 0
        # for i in range(len(self.entropy_list)):
        #
        #     aux = self.entropy_list[i]
        #     size += len(inputs[0])
        #     print("ola", aux)


        # current_average_entropy = sum(self.entropy_list)/len(self.entropy_list)
        # if out_entropy <= current_average_entropy:
        # #     #out_sum = out_sum * tf.repeat([1.2, 1.1, 1.05, 1.3, 1, 1.3, 0.9], repeats=[len(inputs[0])], axis=0)
        #     pass
...
class POI_RGNNE(NNBase):
    ...
    def build(self, step_size, location_input_dim, time_input_dim, num_users, seed=None):
        ...
        adjacency_week_matrix = Input((location_input_dim, location_input_dim), dtype='float32', name='adjacency_week_matrix')
        adjacency_weekend_matrix = Input((location_input_dim, location_input_dim), dtype='float32', name='adjacency_weekend_matrix')
        categories_durations_week_matrix = Input((location_input_dim, location_input_dim), dtype='float32',
                                            name='categories_durations_week_matrix')
        categories_durations_weekend_matrix = Input((location_input_dim, location_input_dim), dtype='float32',
                                                 name='categories_durations_weekend_matrix')

        categories_durations_matrix = Input((location_input_dim, location_input_dim), dtype='float32', name='categories_durations_matrix')
        categories_distance_week_matrix = Input((location_input_dim, location_input_dim), dtype='float32',
                                            name='categories_distance_week_matrix')
        categories_distance_weekend_matrix = Input((location_input_dim, location_input_dim), dtype='float32',
                                                 name='categories_distance_weekend_matrix')
        ...
        x_durations_week = GCNConv(22, activation='swish')([categories_durations_week_matrix, adjacency_week_matrix])
        x_durations_week = Dropout(0.5)(x_durations_week)
        x_durations_week = GCNConv(10, activation='swish')([x_durations_week, adjacency_week_matrix])
        x_durations_week = Dropout(0.5)(x_durations_week)
        x_durations_week = Flatten()(x_durations_week)

        x_durations_weekend = GCNConv(22, activation='swish')([categories_durations_weekend_matrix, adjacency_weekend_matrix])
        x_durations_weekend = Dropout(0.5)(x_durations_weekend)
        x_durations_weekend = GCNConv(10, activation='swish')([x_durations_weekend, adjacency_weekend_matrix])
        x_durations_weekend = Dropout(0.5)(x_durations_weekend)
        x_durations_weekend = Flatten()(x_durations_weekend)
        ...
        x_distance_week = GCNConv(22, activation='swish')([categories_distance_week_matrix, adjacency_week_matrix])
        x_distance_week = Dropout(0.5)(x_distance_week)
        x_distance_week = GCNConv(10, activation='swish')([x_distance_week, adjacency_week_matrix])
        x_distance_week = Dropout(0.5)(x_distance_week)
        x_distance_week = Flatten()(x_distance_week)

        x_distance_weekend = GCNConv(22, activation='swish')([categories_distance_weekend_matrix, adjacency_weekend_matrix])
        x_distance_weekend = Dropout(0.5)(x_distance_weekend)
        x_distance_weekend = GCNConv(10, activation='swish')([x_distance_weekend, adjacency_weekend_matrix])
        x_distance_weekend = Dropout(0.5)(x_distance_weekend)
        x_distance_weekend = Flatten()(x_distance_weekend)
